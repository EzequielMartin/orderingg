...
def order_product_detail(pk_order, pk_product):
    ...
    if request.method == 'GET':
        ...
        return jsonify(order_product.serialize)
"""
Obtiene el producto de una orden y se usa el metodo HTML DELETE para asi poder eliminarlo
"""
@app.route("/order/<pk_order>/product/<pk_product>", methods=['DELETE'])
def borrarProducto(pk_order, pk_product):

    order_product = OrderProduct.query.filter(and_(OrderProduct.order_id==pk_order, OrderProduct.product_id==pk_product)).all()[0] #Obtengo producto
